...
fun NutritionSection(recipe: RecipeDetail) {
    ...
    Row(
        ...
        calories?.let { NutrientBox("${it.amount?.toInt()} kcal", "calories", PinkLight, PinkDark, Modifier.weight(1f)) }
        protein?.let { NutrientBox("${it.amount?.toInt()} g", "protein", BlueLight, BlueDark, Modifier.weight(1f)) }
        carbs?.let { NutrientBox("${it.amount?.toInt()} g", "carbs", GreenLight, GreenDark, Modifier.weight(1f)) }
        fat?.let { NutrientBox("${it.amount?.toInt()} g", "fat", OrangeLight, OrangeDark, Modifier.weight(1f)) }
    }
}